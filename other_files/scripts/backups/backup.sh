...
VAULTWARDEN_BACKUP_FOLDER="/bit_server/vaultwarden/data"
VAULTWARDEN_IGNORE_LIST=("db.sqlite3*" "tmp" "icon_cache")
VAULTWARDEN_SQLITE_DB="/bit_server/vaultwarden/data/db.sqlite3"
...
[[ $ONLY_UPLOAD -eq 0 ]] && env \
    VERBOSE_OVERRIDE=$VERBOSE \
    DATE_OVERRIDE=$DATE \
    BACKUP_FOLDER=$VAULTWARDEN_BACKUP_FOLDER \
    LONG_TERM_BACKUP_FOLDER=$VAULTWARDEN_LONG_TERM_BACKUP_FOLDER \
    IGNORE_LIST=$( IFS=:; printf '%s' "${VAULTWARDEN_IGNORE_LIST[*]}" ) \
    SQLITE_DB_PATH=$VAULTWARDEN_SQLITE_DB \
    TEMP_FOLDER=$TEMP_FOLDER \
    bash "${args[@]}" $BACKUP_SCRIPT_FOLDER/vaultwarden-backup.sh
#
...
#     VAULTWARDEN_LONG_TERM_BACKUP_FOLDER=$VAULTWARDEN_LONG_TERM_BACKUP_FOLDER \
#     SECRETS_LONG_TERM_BACKUP_FOLDER=$SECRETS_LONG_TERM_BACKUP_FOLDER \
#     GDRIVE_ACCOUNT=$GDRIVE_ACCOUNT \
#     GDRIVE_CREDENTIALS_PATH=$GDRIVE_CREDENTIALS_PATH \
